...
def mutate_tensor(x, indices, values):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  for item_idx, item  in enumerate(indices):
    x[item[0], item[1]] = values[item_idx]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def multiples_of_ten(start, stop):
  ...
  mul=10
  tensor_list = []
  while(start <= mul <= stop):
    tensor_list.append(mul)
    mul = mul + 10
  
  x = torch.tensor(tensor_list, dtype=torch.float64)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop:
    ...
    for idx in range(x.shape[0]):
      new_m = torch.mm(x[idx, :, :], y[idx, :, :])
      tensors.append(new_m)
...
def normalize_columns(x):
  ...
  y = x.clone()
  M = y.shape[1]
  sum_c = torch.sum(y, dim=0)
  mean_c = sum_c / M
  std_c = torch.sqrt(torch.sum((y - mean_c)**2, dim=0) / (M - 1))
  y = y - mean_c
  y = y / std_c
  #############################################################################
  #                            END OF YOUR CODE                               #
