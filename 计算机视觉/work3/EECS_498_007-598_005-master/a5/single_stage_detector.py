...
def GenerateAnchor(anc, grid):
  ...
  anchors = torch.zeros((B, A, H, W, 4)).to(grid.device).type(grid.dtype)
  anchors[:, :, :, :, :] = center.reshape(B, 1, H, W, 4)
...
def GenerateProposal(anchors, offsets, method='YOLO'):
  ...
  xc_a = anchors[:, :, :, :, 0] + (w_a / 2)
  h_a = anchors[:, :, :, :, 3] - anchors[:, :, :, :, 1]
  yc_a = anchors[:, :, :, :, 1] + (h_a / 2)

  xc_c = xc_a
  yc_c = yc_a
  w_c = w_a * torch.exp(offsets[:, :, :, :, 2])
  h_c = h_a * torch.exp(offsets[:, :, :, :, 3])
  ...
  proposals[:, :, :, :, 0] = xc_c - (w_c / 2)
  proposals[:, :, :, :, 2] = xc_c + (w_c / 2)
  proposals[:, :, :, :, 1] = yc_c - (h_c / 2)
  proposals[:, :, :, :, 3] = yc_c + (h_c / 2)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def IoU(proposals, bboxes):
  ...
  B, N, _ = bboxes.shape
  K = A * H * W
  W_p = proposals[:, :, :, :, 2] - proposals[:, :, :, :, 0]
  H_p = proposals[:, :, :, :, 3] - proposals[:, :, :, :, 1]
  aop = H_p * W_p # B A H W
  aop = aop.reshape(B, K, 1) # to be broadcasted to B K N 

  W_b = bboxes[:, :, 2] - bboxes[:, :, 0] 
  H_b = bboxes[:, :, 3] - bboxes[:, :, 1]
  aob = H_b * W_b # B N
  aob = aob.reshape(B, 1, N) # to be broadcasted to B K N 

  l1_x = proposals[:, :, :, :, 0].reshape(B, K, 1)
  r1_x = proposals[:, :, :, :, 2].reshape(B, K, 1)
  l1_y = proposals[:, :, :, :, 1].reshape(B, K, 1)
  r1_y = proposals[:, :, :, :, 3].reshape(B, K, 1)
  l2_x = bboxes[:, :, 0].reshape(B, 1, N)
  r2_x = bboxes[:, :, 2].reshape(B, 1, N)
  l2_y = bboxes[:, :, 1].reshape(B, 1, N)
  r2_y = bboxes[:, :, 3].reshape(B, 1, N)

  x_dist = torch.minimum(r1_x, r2_x) - torch.maximum(l1_x, l2_x)
  x_dist[x_dist < 0] = 0
  y_dist = torch.minimum(r1_y, r2_y) - torch.maximum(l1_y, l2_y)
  y_dist[y_dist < 0] = 0
...
class PredictionNetwork(nn.Module):
  def __init__(self, in_dim, hidden_dim=128, num_anchors=9, num_classes=20, drop_ratio=0.3):
    ...
    self.pred_layer = nn.Sequential(OrderedDict([
      ("conv1", nn.Conv2d(in_dim, hidden_dim, kernel_size=1, padding=0, stride=1)),
      ("dropout", nn.Dropout(drop_ratio)),
      ("leaky_relu", nn.LeakyReLU()),
      ("conv2", nn.Conv2d(hidden_dim, 5*A+C, kernel_size=1, padding=0, stride=1)),
    ]))
  ...
  def forward(self, features, pos_anchor_idx=None, neg_anchor_idx=None):
    ...
    conf_scores = out[:, :5*A:5, :, :] # (B X A X H X W)
    ...
    offsets[:, :, 0:2, :, :] = torch.sigmoid(offsets[:, :, 0:2, :, :]) - .5

    class_scores = out[:, 5*A:, :, :] # (B X C X H X W)
    if pos_anchor_idx is not None and neg_anchor_idx is not None:
      conf_scores = conf_scores.view(B, A, 1, H, W) # (B X A X 1 X H X W)
...
class SingleStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    pos_anchor_idx, neg_anchor_idx, GT_conf_scores, GT_offsets, GT_class, \
      ...
      bboxes, grid, iou_mat, neg_thresh=0.2, method='YOLO')
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    conf_scores = conf_scores.detach()
    offsets = offsets.detach()
    class_prob = class_prob.detach()

    for b in range(B):
      conf_val, conf_idx = conf_scores[b].flatten(start_dim=1).max(dim=0) # HW
      ...
      _, class_t = class_prob[b].flatten(start_dim=1).max(dim=0) # HW
      class_t = class_t[mask]
      class_t = class_t[kept].view(-1, 1)

      final_proposals.append(proposals_t)
      final_conf_scores.append(conf_scores_t)
      final_class.append(class_t)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def nms(boxes, scores, iou_threshold=0.5, topk=None):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  new_scores = scores.clone().to(scores.device).type(scores.dtype)
  orig_idx = torch.arange(new_scores.shape[0])
  new_boxes = boxes.clone().to(boxes.device).type(boxes.dtype)
  keep = None

  while orig_idx.shape[0] != 0:
    _, max_idx = new_scores.max(dim=-1)
    new_scores = torch.cat((new_scores[:max_idx], new_scores[max_idx+1:])) # pop it
    
    max_orig_idx = torch.tensor([orig_idx[max_idx]])
    if keep is None:
      keep = max_orig_idx
    else:
      keep = torch.cat((keep, max_orig_idx)) # push it
    orig_idx = torch.cat((orig_idx[:max_idx], orig_idx[max_idx+1:])) # pop it

    max_box = new_boxes[max_idx, :] # 4
    new_boxes = torch.cat((new_boxes[:max_idx, :], new_boxes[max_idx+1:, :])) # pop it
    max_box = torch.ones_like(new_boxes).view(-1, 1, 4) * max_box
    new_boxes = new_boxes.view(-1, 1, 1, 1, 4)
    iou_max = IoU(new_boxes, max_box).flatten()
    mask = (iou_max <= iou_threshold)

    new_scores = new_scores[mask]
    orig_idx = orig_idx[mask]
    new_boxes = new_boxes.view(-1, 4)[mask, :]
