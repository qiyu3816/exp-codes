...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes, fc_w_second_dim
  , dtype=dtype, device=device))
...
class ResidualBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block = nn.Sequential(OrderedDict([
      ("plainblock", PlainBlock(Cin, Cout, downsample))
    ]))
    ...
    if Cin == Cout:
      ...
      if downsample:
        self.shortcut = nn.Sequential(OrderedDict([
          ('conv2', nn.Conv2d(Cin, Cout, 1, stride=2))
        ]))
      else:
        self.shortcut = nn.Sequential(OrderedDict([
          ('conv2', nn.Conv2d(Cin, Cout, 1))
        ]))
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
